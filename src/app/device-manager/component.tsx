...
import SimpleBackdrop from '../generic/simple-backdrop/component';
...
const DeviceManager: React.FC = () => {
  ...
  const handleNoteOn = useCallback(
    (event: InputEventNoteon) => {
      gameDispatch({ type: 'play-note', note: event.note.number });
    },
    [gameDispatch],
  );
  ...
  const handleNoteOff = useCallback(
    (event: InputEventNoteoff) => {
      gameDispatch({ type: 'rest-note', note: event.note.number });
    },
    [gameDispatch],
  );
